...
class IGraphDataset(InMemoryDataset):
    ...
    def init_graph_labels(self):
        ...
        self.labels = torch.IntTensor(encoded_labels)
    ...
    def get_edges_from_igraph(self, iG: ig.Graph) -> torch.LongTensor:
        ...
        assert pyg_edge_index.shape == (2, e_num)
        return pyg_edge_index

    def get_edge_arrts_from_igraph(self, iG: ig.Graph) -> Optional[torch.FloatTensor]:
    ...
    def get_node_arrts_from_igraph(self, iG: ig.Graph) -> Optional[torch.FloatTensor]:
    ...
    def convert_raw_graphs(self):
        ...
        for i, graph_path in enumerate(
            ...
            pyg_edge_attrs = self.get_edge_arrts_from_igraph(iG)
            pyg_x = self.get_node_arrts_from_igraph(iG)
